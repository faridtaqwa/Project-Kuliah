 ...
 class Simple_login_trc {
     ...
     */
     public function login($username, $password) {
         ...
         $query = $this->CI->db->get_where('trc',array('username'=>$username,'password' => md5($password)));
 
         if($query->num_rows() == 1) {
             //ambil data user berdasar username
             $row  = $this->CI->db->query('SELECT id FROM trc where username = "'.$trc.'"');
             $admin     = $row->row();
             $id   = $admin->id;
             ...
             redirect(site_url('trc/dashboard'));
         }else{
