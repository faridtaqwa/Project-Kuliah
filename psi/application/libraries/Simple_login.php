 ...
 class Simple_login {
     ...
     */
     public function login($email, $password) {
         ...
         $query = $this->CI->db->get_where('pusdalops',array('email'=>$email,'password' => md5($password)));
 
         if($query->num_rows() == 1) {
             //ambil data user berdasar username
             $row  = $this->CI->db->query('SELECT id FROM pusdalops where email = "'.$email.'"');
             $admin     = $row->row();
             $id   = $admin->id;
             ...
             redirect(site_url('pusdalops/dashboard'));
         }else{
             ...
             redirect(site_url('pusdalops/login'));
         }
          return false;
      }
